 ...
 */
export function get<T = ConfigValue>(obj: ConfigObject, path: string, defaultValue?: T): T {
    if (!path) {
...
export function mergeConfig(target: ConfigObject, source: ConfigObject): ConfigObject {
    ...
    for (const key in source) {
        ...
        if (isPlainObject(sourceValue) && isPlainObject(targetValue)) {
            result[key] = mergeConfig(targetValue as ConfigObject, sourceValue as ConfigObject);
        } else {
 ...
 * @param prefix 前缀
 */
export function flatten(obj: ConfigObject, prefix = ''): Record<string, ConfigValue> {
 ...
 */
export function unflatten(flattened: Record<string, ConfigValue>): ConfigObject {
 ...
 */
export function validateConfigType(value: ConfigValue, expectedType: string): boolean {
    switch (expectedType) {
        case 'string':
