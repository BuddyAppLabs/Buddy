<script setup lang="ts">
  ...
  const {
    ...
    height,
    update: updateBounds,
  } = useElementBounding(embeddedViewContainer);
  ...
  const debouncedUpdateViewBounds = useDebounceFn(async () => {
    // 只有在嵌入式视图时才更新
    if (
      viewState.value.embedded.isAttached &&
      viewState.value.currentAction?.viewPath
    ) {
      updateBounds();
  ...
  const { start: startCreateEmbeddedView } = useTimeoutFn(
    createEmbeddedViewImmediate,
    100
  );
  ...
  async function createEmbeddedViewImmediate() {
    ...
    try {
      ...
      logger.info(
        `PluginView: 创建嵌入式视图: ${viewState.value.embedded.id}，容器边界: ${JSON.stringify(bounds)}`
      );
...
<template>
  <div class="plugin-view flex flex-col h-full">
    ...
    <div
      ...
      style="min-height: 400px; position: relative; z-index: 1"></div>

    <!-- 插件使用独立窗口显示 -->
    <div
      v-if="!viewState.embedded.isAttached && viewState.window.isOpen"
