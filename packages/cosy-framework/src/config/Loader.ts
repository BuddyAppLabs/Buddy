...
export class Loader implements ConfigLoader {
     ...
     * @param options 缓存选项
     */
    async cache(config: ConfigObject, options: ConfigCacheOptions): Promise<void> {
        if (!options.path) {
     ...
     * @param options 缓存选项
     */
    async loadFromCache(options: ConfigCacheOptions): Promise<ConfigObject | null> {
        if (!options.path) {
            return null;
        }
     ...
     */
    private async scanConfigFiles(configPath: string): Promise<ConfigFileDefinition[]> {
     ...
     * @param file 配置文件定义
     */
    private async loadConfigFile(file: ConfigFileDefinition): Promise<ConfigObject> {
