...
export function AuthMiddleware(options: {
    required?: boolean;
    permissions?: string[];
    authenticator?: (event: IpcMainInvokeEvent) => Promise<AuthContext> | AuthContext;
} = {}): Middleware {
    const {
        required = true,
        permissions = [],
 ...
 */
async function defaultAuthenticator(event: IpcMainInvokeEvent): Promise<AuthContext> {
