...
export class PluginFacade {
   ...
   * 获取插件管理器实例
   */
  private static getManager(): IPluginManager {
    return this.app.make<IPluginManager>('plugin');
  }
   ...
   * 获取动作视图
   */
  public static async getActionView(actionId: string): Promise<string> {
    return await this.getManager().getActionView(actionId);
  }
}
