...
export abstract class BasePluginRepo implements PluginRepoContract {
    ...
    public async getAllPlugins(): Promise<PluginEntity[]> {
        if (verbose) {
            LogFacade.channel('plugin').info(`[BasePluginRepo] 获取插件列表，根目录是`, { rootDir: this.rootDir });
        }
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                try {
                    ...
                    LogFacade.channel('plugin').warn(`[BasePluginRepo] 读取插件信息失败`, { error: error instanceof Error ? error.message : String(error) });
                }
            }
            ...
            if (validPlugins.length === 0) {
                ...
                if (verbose) {
                    LogFacade.channel('plugin').info(`[BasePluginRepo] 有效的插件数量` + validPlugins.length, { validPlugins: validPlugins.length });
                }
            }
            ...
            LogFacade.channel('plugin').error(`[BasePluginRepo] 获取插件列表失败`, { error: error instanceof Error ? error.message : String(error) });
            return [];
        }
    }
    ...
    public async find(id: string): Promise<PluginEntity | null> {
        try {
            ...
            LogFacade.channel('plugin').error(`[BasePluginRepo] 查找插件失败: ${id}`, { error: error instanceof Error ? error.message : String(error) });
            return null;
        }
    }
