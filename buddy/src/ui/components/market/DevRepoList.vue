...
<template>
    ...
    <div class="mb-4 w-full">
        <div class="flex items-center justify-between p-2 rounded-md bg-base-200 text-sm w-full">
            <span>当前开发仓库目录:
    ...
    <template v-if="marketStore.devPluginDirectory">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <template v-if="devPlugins.length > 0">
                <PluginCard v-for="plugin in devPlugins" :key="plugin.id" :package="plugin" type="remote" />
            </template>
            <Alert v-else type="warning" title="没有找到插件" :closable="false" />
        </div>
    </template>

    <Alert v-else type="warning" title="请先配置开发仓库目录" :closable="false" />
</template>
