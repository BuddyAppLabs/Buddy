...
<template>
  <div class="flex items-center p-3 transition-colors duration-200 rounded-md" :class="{
    ...
    <button v-if="actionable && !loading"
      class="px-2 py-1 rounded bg-primary text-primary-content border-none cursor-pointer" @click.stop="handleAction">
      {{ actionText || '操作' }}
    </button>
  </div>
</template>
