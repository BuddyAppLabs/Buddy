...
<template>
  <transition name="card-fade" appear enter-active-class="transition-all duration-300 ease"
    ...
    <div class="p-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-300" :class="cardClass">
      ...
      <!-- 插件基本信息 -->
      <p class="text-sm mb-4">{{ plugin.description }}</p>
      ...
      <div class="mt-4 flex flex-wrap gap-2 items-center" v-if="plugin.type != 'dev'">
        ...
        <template v-if="plugin.type === 'user'">
          ...
          <div v-if="isUserPlugin">
            ...
            <Button variant="primary" @click="confirmUninstall"
              ...
              {{
                uninstallingPlugins.has(plugin.id) || isUninstalling
                  ? '卸载中...'
                  : '卸载'
              }}
            </Button>
        ...
        <template v-if="plugin.type == 'remote'">
          ...
          <!-- 下载成功提示 -->
          <transition name="fade" enter-active-class="transition-opacity duration-300 ease" enter-from-class="opacity-0"
            enter-to-class="opacity-100" leave-active-class="transition-opacity duration-300 ease"
