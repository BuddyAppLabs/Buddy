    ...
    <div v-if="errorStore.isErrorModalVisible" class="modal modal-open z-[999]">
        <div class="modal-box flex max-w-2xl flex-col !overflow-hidden">
            ...
            <div class="my-4 flex-1 space-y-2 overflow-y-auto">
                <Alert v-for="error in errorStore.errors" :key="error.id" :type="error.type" :title="error.message"
                    :closable="true" @close="errorStore.removeError(error.id)" />
            </div>

            <div class="modal-action">
                <Button v-if="errorStore.errors.length > 0" @click="errorStore.clearErrors()">清除全部</Button>
            </div>
        </div>
    </div>
</template>
